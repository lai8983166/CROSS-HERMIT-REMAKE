...
- [x] 1.1 `tools/map_export.py`：参数=地图编号（`01`），读
...
- [x] 1.2 损坏输入护栏：构造尺寸不符的假 BIN（临时文件）运行工具 → 非零退出 + 报错信息。
       验证：stderr 有信息、目标目录无新 JSON

## 2. sim 数据层

- [x] 2.1 `prototype/sim/map_data.gd`（class_name SimMapData）：加载 map JSON、
       `cell_w/cell_h` 等维度、`layer(name,x,y)`（越界 -1）、`cell_to_world/world_to_cell`
       （32×16 菱形，含格心偏移）。验证：GUT——加载 map01 后 terrain(0,0)==JSON 值、
       全网格换算往返、越界 -1 三则
- [x] 2.2 换算实现与 main.gd 既有 ORIGIN/菱形画法对齐（同一 cell_to_world 驱动绘制）。
