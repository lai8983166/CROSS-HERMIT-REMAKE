...
目前这个工具只审计已有导出链，**不**证明技能调度时序、EFCT 的施放位置、目标阵营、伤害生效时刻，也不替代实机差分。它的直接用途是定位编号关联的断点：例如技能 22 的 `2044`/`2029` 属于有效但未导出的 EFCT 动画；action 31 已按原版动作表导出，但 E0A/E1A 的原始程序缺失；action 13 的北向映射在 C0A 有、D0A 没有，因此不能跨单位直接套用。
