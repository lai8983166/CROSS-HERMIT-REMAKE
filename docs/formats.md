...
"菱形观感"来自贴片绘制而非坐标投影。地图头部的 255×4B 三元组表 (a,b,c 对角爬升) 与 53×(u16,u16) 步长表语义**仍开口** (疑高度/条带数据, 非贴图映射)。
```
...
### 开口
- [ ] BIN 三层语义对照实机截图 (直角定案后: terrain/variant 疑非贴图选择器——世界图已在图集内, 层疑为碰撞/高度/物件标记)
- [ ] 255×4B 索引表语义
