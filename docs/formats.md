...
- **`##E.BIN` (01E..53E) = 特效档**: 全档单色相 (如 01E = 品红斩击 + 蓝色光球), E = Effect。同容器 9 块格式; 块0 动画为**播一次序列** (首帧 dur=20 定格 + 后续 dur=6 降序帧串); 01E 实测: #14..#28 帧引用把 5..93 分成连续带 = 各特效帧带 (视觉标注: #15=紫月斩 #22=刀光展开 #24=多段斩 #28=弹丸), #1..#13 全为外部引用 (帧号 ≥ 本档帧数, 跨档续编语义开口) — tools/fx_export.py
...
- **单位档身份表 (视觉鉴定 2026-09-18)**: A0A=剑盾轻步兵 | A1A/D1A=烟雾怪 | B0A=待机帧为魔法阵
  (身份存疑) | B1A=大幽灵怪 | C0A=男弓手 | C1A=女弓手 | D0A=法师 | E0A=细剑士/盗贼 |
  E1A=最壮的持棍战士 (重战士最佳近似)。引擎侧 单位/职业→档 映射字段仍未定位 (开口);
  重制侧 battle_setup.json 的 anim_id 手配即用
