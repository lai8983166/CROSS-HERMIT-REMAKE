...
纹理页: pageW×pageH × 256×256 u16, 步长 0x2000C (页+12B "DX"2,256,256,256,256 定长标记)
  (MAP01: 0x4D8 起 48 页; MAP02: 0x404 起 108 页; MAP01 末页后另有 832B 尾巴, 未解)
尾部: 变长结构数据 (MAP01 0x234B)
```
...
### 开口
- [ ] 大纹理 this+0xdc 的构造处 (48 页→2048×1536 拼合代码, 疑 43b8b0..43be60 区; 绘制层 UV 已证恒等, 不阻塞)
- [ ] 缝带取样源 (48px 竖直缝带 @图集 x≈592/870, 需第二相机位置截图; 引擎管线已明, 缝带疑为大纹理构造时非行优先细节)
- [ ] BIN 层值语义: terrain {1..5,9..13,17} / variant 35 格非零 (引擎代码证明非贴图选择器——地面=大纹理恒等; 疑战术/碰撞/高度)
- [ ] 阴影动态算法 (43d270 及邻近, 运行时计算非文件)
- [ ] MAP01 尾部 832B (末页之后) + 头区 0x10..0x404 的 255×4B 三元组表
