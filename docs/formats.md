...
体: cellW×cellH × 3 层 u16 (MAP01 = 6144 格 × 6B, 共 36,880 ✓)
  三层均为小整数 0~12、~2/3 为 0 (层0/1/2 = 地形类别/贴图变体/物件, 待截图对照)
...
纹理页: pageW×pageH × 256×256 u16, 步长 0x20000 (MAP01: 0x4D8 起 48 页; MAP02: 0x404 起 108 页)
尾部: 变长结构数据 (MAP01 0x234B)
```
- 页定位法: dist=512 字节相关 >30% (行重复签名); 像素格式 = **X1RGB555** (与 §1 全引擎 DX 图像
...
### VPT (寻路容器, 标准偏移表容器)
```
[u32 总长=文件大小][u32 n=8][u32 偏移[9]]  数据从 0x28 起
条目 0~3: 0x2108 B 序列化图结构 (串含 MapEditFlIDCode; +0x14=指针数, +0x18 起相对指针
          由 44e5d0 重定位 +基址)
条目 4~7: 0x3000 B = cellW×cellH×2 全格 u16 层 ×4 (与 BIN 同维度)
```

### 开口
- [ ] BIN 三层语义对照实机截图
- [ ] VPT 0x2108 图结构的节点/边布局 (寻路算法复刻时需要)
- [ ] 255×4B 索引表语义
