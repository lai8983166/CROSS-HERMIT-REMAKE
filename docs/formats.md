...
## 10. DxAnim 动画容器 (DATA/DXANIM/*.BIN) ✅运行时链已定案 (2026-09-19, tools/unit_anim_export.py)

文件分三类 (色相量化 + 视觉目检定案):
- **`{A..E}{0,1}A.BIN` (10 档) = 单位精灵**: 每档一个角色全动作集 (本体+挥砍特效混装),
  121 动画 × 138~251 帧, 帧最大 215×280, 多色相 (肤/发/衣料)
- **`##E.BIN` (01E..53E) = 特效档**: 与单位档使用相同的 9 块程序/描述符结构。
  01E 正确解码后，#21 在蓝色本体动作中并发生成紫色斩击层，#28 为蓝色旋转体；旧的
  “#15=帧5..10 斩击、#28=帧88..93 光球”结论来自把打包描述符误当 BMP 帧号，已废弃。
- **`EFCT.BIN`** = 原版战斗初始化实际加载的 7 块特效合集（`0x466000`）；全局动画号按
  `block = id / 1000 - 1`、`animation = id % 1000` 解析，例如 2042→块1/#42、3027→块2/#27。
  `TACTICS/COMMON.BIN` (580 条) = 头像 50×50×96 +
...
| 块 | 语义 | 状态 |
|---|---|---|
| 0 | 动画程序表；每条指令 10B，低2位 opcode。opcode0 的 bytes2..5 + bytes6..7 四个半字节组成最多四个 12 位块4描述符号，bytes8..9 为持续 tick | ✅ |
| 1 | 与块0同格式的动画程序表 | ✅ |
| 2 | (常为空块) | — |
| 3 | opcode1 创建的子动画程序表；与父实例并发，继承并累积局部偏移 | ✅ |
| 4 | **10B 绘制描述符直排**：`i16 x,y,frame; u8 flags,palette; i16 draw_group`；由块0～3 opcode0 的 12 位索引引用 | ✅ |
...
- **程序指令 (定案 2026-09-19)**: opcode0 显示块4描述符并等待；opcode1 以 `(动画号,dx,dy)`
  创建块3子实例；opcode2 触发音效（导出侧跳过副作用）；opcode3 按 repeat/jump/方向位控制循环。
  0x80 是终止位，0x40 是 opcode3 前/后跳转方向。子实例与父实例必须按同一 tick 合成，不能串行展平。
- **汇编链闭合 (2026-09-19)**: 0x47dec0 移动调用 `0x465040(..., no=3, dir)`；0x465040
  从动作/方向表取得高字节动画号、低字节块号，并把表项第二字节低两位作为独立镜像 flags 传给
  0x409ff0。移动表高字节已是块0实际下标 #11～#15，不存在 `+5` 转换，也不因镜像进入块1。
  调色板选择字段亦闭合: 0x409b70 `pindex≥1 → 块7[pindex-1]` (单位表 0x610538 第二字段 0x20000|slot 的 slot)
- **移动八向映射**: 角度表给出 d6=E、d3=SE、d2=S、d1=SW、d4=W、d7=NW、d8=N、d9=NE
  （d5 为中心位）。动作 `no=3` 因而映射为 N→#11、NW/NE→#12（NE 镜像）、W/E→#13
  （E 镜像）、SW/SE→#14（SE 镜像）、S→#15。walk_by_dir8 已写进
  unit_sprites.json (W/E/N/S/NW/NE/SW/SE → 块0槽位+镜像位), 视图按移动向量 8 向选带
- **技能动作 11～16**: 每个动作占块0连续 5 个程序，基址为 `action*5-4`；N=+0、NW/NE=+1、
  W/E=+2、SW/SE=+3、S=+4，东向三项使用 x 镜像。不同单位档允许动作槽为空，导出器在
  `anim_map.skill_actions` 中省略空方向，运行时必须安全回退待机。
- A0A 黄金链: #11～#15 的描述符分别为 569..574、563..568、557..562、551..556、545..550，
  最终 BMP 帧分别为 244..249、238..243、232..237、226..231、220..225；B1A 对应最终帧
  224..229、218..223、212..217、206..211、200..205。
- 锚点定案: **画布底中 = 脚底**, 帧内偏移 = `(画布w/2 − 帧x, 画布h − 帧y)` (数据推导)
- 导出: `tools/unit_anim_export.py` → `prototype/assets/unit/<档>/frame_*.png + _preview.png` +
  `prototype/data/unit_sprites.json`。schema v2 的 `steps[].layers[]` 保存持续 tick、多图层、局部偏移、
  镜像和原描述符属性；`loop_from` 指向循环 step。JSON 仍可直接修改，运行时兼容旧单帧记录。
- 特效导出: `tools/fx_export.py` 只从 `EFCT.BIN` 按全局 ID 导出实际引用帧到
  `prototype/assets/fx/EFCT/`。持续施法动画会重复生成短命块3子实例；循环状态签名不能包含实例
  分配序号，否则永不收敛。2044～2051 均已验证具有稳定 `loop_from`；3017 是 120 tick 的空白
  同步动画，空图层仍必须保留其时长。
- 开口: ATTACK/DEAD 的完整动作标签尚未定案; UNITPAL.BIN 70 调色板疑为
