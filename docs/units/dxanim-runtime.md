...
普通攻击 101～104 的技能明细表 `0x611538` 在 byte +1 指定动作号 `no=5`。
`UnitAnim.cpp` 仍通过同一方向表选择 block 0 动画，因此八方向映射为：
...
A0A 的 action 5 为 8 步、36 tick，B1A 为 9 步、48 tick；其中武器挥砍/残影是
单位时间线自身的复合图层，不应再叠加通用 `##E`。职业 1/3 的默认攻击 101/103
在技能明细表 +0x0A/+0x0C 的投射/命中动画均为 0，攻击表 +0x1A 的 hit effect 也为 0，
因此原版普通攻击没有独立外置特效。运行时只为非零且已完成语义映射的效果 ID 播放 `##E`。

## 黄金样例

- A0A #11～#15 最终帧：`244..249`、`238..243`、`232..237`、`226..231`、`220..225`。
- B1A #11～#15 最终帧：`224..229`、`218..223`、`212..217`、`206..211`、`200..205`。
- A0A action 5 #21～#25 的持续时间均为 36 tick，并包含挥砍复合层。
- B1A action 5 #21～#25 的持续时间均为 48 tick，并包含挥砍复合层。
...
导出验证要求：10个单位档均有八方向 MOVE 与 ATTACK；所有 layer 帧号均小于对应 `frame_count`；
